...
export default function QuizGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    setOutput('');
    try {
      const { data } = await axios.post(`${API}/generate/quiz`, form, { withCredentials: true });
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl mx-auto animate-fade-in" data-testid="quiz-page">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-400">
          Generate quizzes with answer keys and progressive difficulty scaling.
        </p>
      </div>

      <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6 md:p-8 mb-8" data-testid="quiz-form">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <Label className="text-sm text-slate-300">Subject *</Label>
            <Input
              data-testid="quiz-subject-input"
              placeholder="e.g., Biology, Physics, English"
              value={form.subject}
          ...
          <div className="space-y-2">
            ...
            <Select value={form.grade_level} onValueChange={(v) => setForm({ ...form, grade_level: v })}>
              <SelectTrigger data-testid="quiz-grade-select" className="bg-black/20 border-white/10 text-white focus:border-violet-500 focus:ring-1 focus:ring-violet-500">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Input
              ...
              placeholder="e.g., Cell Division, Newton's Laws, Shakespeare"
              value={form.topic}
          ...
          <div className="space-y-2">
            ...
            <Select value={form.difficulty} onValueChange={(v) => setForm({ ...form, difficulty: v })}>
              <SelectTrigger data-testid="quiz-difficulty-select" className="bg-black/20 border-white/10 text-white focus:border-violet-500 focus:ring-1 focus:ring-violet-500">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-[#120E2C] border-white/10">
        ...
        <Button
          ...
          className="mt-6 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl px-8 py-2.5 font-medium disabled:opacity-50"
        >
          {loading ? (
            <><Loader2 size={16} className="mr-2 animate-spin" />Generating...</>
          ) : (
            <><Sparkles size={16} className="mr-2" />Generate Quiz</>
          )}
        </Button>
      </div>
      ...
      {loading && (
        <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8 mb-8" data-testid="loading-state">
          ...
          <p className="text-center text-sm text-slate-500 mt-6">AI is crafting your quiz...</p>
        </div>
      )}

      {output && <OutputDisplay content={output} type="Quiz" testId="quiz-output" />}
    </div>
  );
}
